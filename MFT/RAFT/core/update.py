...
class SmallUpdateBlock(nn.Module):
    def __init__(self, corr_levels: int, corr_radius: int, hidden_dim: int = 96):
...
class OcclusionAndUncertaintyBlock(nn.Module):
    def __init__(
        self,
        occlusion_module: str,
        occlusion_input_detach: bool,
        uncertainty_input_detach: bool,
        hidden_dim: int = 96,
    ):
        super(OcclusionAndUncertaintyBlock, self).__init__()
        self.occlusion_module = occlusion_module

        architecture = "simple"
        if "morelayers" in occlusion_module:
            architecture = "morelayers"
        self.occlusion_detach = occlusion_input_detach
        self.uncertainty_detach = uncertainty_input_detach

        self.occl_head = OcclusionHead(hidden_dim, hidden_dim=128, architecture=architecture)

        if "with_uncertainty" in occlusion_module:
            if "separate" in occlusion_module:
                self.uncertainty_head = UncertaintyHead(
                    hidden_dim, hidden_dim=128, architecture=architecture
                )
            else:
                raise NotImplementedError(
                    f"Type {occlusion_module} of occlusion/uncertainty module is not implemented"
                )
    ...
    def forward(self, net, inp, corr, flow, delta_flow, motion_features):
        ...
        if ("with_uncertainty" not in self.occlusion_module) or (
            "separate" not in self.occlusion_module
        ):
            raise NotImplementedError(
                f"Type {self.occlusion_module} of occlusion/uncertainty module is not implemented"
            )
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, corr_levels: int, corr_radius: int, hidden_dim=128, input_dim=128):
